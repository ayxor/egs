...
docker exec main-keycloak-1 /opt/keycloak/bin/kcadm.sh create users -r egs -s username=professor@ua.pt -s enabled=true -s email=professor@ua.pt -s firstName=Professor -s lastName=User
docker exec main-keycloak-1 /opt/keycloak/bin/kcadm.sh set-password -r egs --username professor@ua.pt --new-password professor
docker exec main-keycloak-1 /opt/keycloak/bin/kcadm.sh update users/$(docker exec main-keycloak-1 /opt/keycloak/bin/kcadm.sh get users -r egs -q username=professor@ua.pt --fields id --format csv --noquotes) -r egs -s 'attributes={"role":["professor"]}'

echo "Adding student user..."
docker exec main-keycloak-1 /opt/keycloak/bin/kcadm.sh create users -r egs -s username=student@ua.pt -s enabled=true -s email=student@ua.pt -s firstName=Student -s lastName=User
